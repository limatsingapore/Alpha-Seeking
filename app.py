...
def calculate_factors(price, volume, min_amt, trading_days=252):
    if len(price) < 60 + 20: return None # 최소 데이터 길이 확인
...
# ==============================================================================
# [데이터 로더: 미국 (Wiki + yfinance)]
# ==============================================================================
@st.cache_data(ttl=3600*24)
def load_us_data(index_name='S&P 500'):
    cfg = {
        'S&P 500': {'url': 'https://en.wikipedia.org/wiki/List_of_S%26P_500_companies', 'cols': {'Symbol': 'Code', 'Security': 'Name'}},
        'NASDAQ 100': {'url': 'https://en.wikipedia.org/wiki/Nasdaq-100', 'cols': {'Ticker': 'Code', 'Company': 'Name'}},
        'DOW 30': {'url': 'https://en.wikipedia.org/wiki/Dow_Jones_Industrial_Average', 'cols': {'Symbol': 'Code', 'Company': 'Name'}}
    }
    try:
        c = cfg.get(index_name)
        tables = pd.read_html(c['url'])
        df = None
        for t in tables:
            if any(k in t.columns for k in c['cols'].keys()):
                df = t; break
        if df is None: return {}, []
        
        rename_map = {k: v for k, v in c['cols'].items() if k in df.columns}
...
# [메인 컨트롤러]
# ==============================================================================
st.title("🌍 Alpha Seeking Pro (Global)")
...
with st.sidebar:
    ...
    if "한국" in market:
        ...
        # 한국: 테마 선택
        # (간소화를 위해 전체 Top 500만 로딩)
        st.info("한국 시장은 유동성 상위 500개 종목을 대상으로 합니다.")
    ...
    st.divider()
    mode = st.radio("실행 모드", ["📊 실시간 분석", "📉 백테스트"])
...
if mode == "📊 실시간 분석":
    ...
    if st.button("분석 실행", type="primary"):
        # 대상 종목 리스트 (한국은 500개, 미국은 선택된 지수)
        targets = ALL_STOCKS
        results = []
        bar = st.progress(0, f"{len(targets)}개 종목 분석 중...")
        
        # 스레드풀로 실시간 데이터 수집 (국가별 라이브러리 차이 처리)
        def worker(t):
            try:
                if COUNTRY == "KR":
                    ...
                    df = yf.Ticker(t).history(period="2y") # 미국은 2년치
        ...
        if results:
            ...
            st.dataframe(disp, use_container_width=True, 
                         column_config={
                             f"Price({CURRENCY})": st.column_config.NumberColumn(format=f"{CURRENCY}%d" if COUNTRY=="KR" else f"{CURRENCY}%.2f"),
...
# -------------------- TAB 2: 백테스트 --------------------
    ...
    st.info(f"⚡ 설정: 수수료 {COST_RATE*100}% | 벤치마크 {BM_NAME} | 최소거래 {MIN_AMT:,.0f}{CURRENCY}")
    
    if st.button("백테스트 시작", type="primary"):
        with st.spinner("10년치 데이터 로딩 및 시뮬레이션 중..."):
            # 유니버스 샘플링 (속도)
            ...
            if not p.empty:
                ...
                if logs:
                    ...
                    # Metrics
                    tot_ret = res['Cum_Port'].iloc[-1] - 1
                    y = len(res)/12
                    cagr = (tot_ret+1)**(1/y)-1 if y>0 else 0
