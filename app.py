...
if scan_button:
    ...
    if len(tickers) > 0:
        ...
        if results:
            ...
            # -----------------------------------------------------------------
            # [수정됨] 서식 사전을 변수로 분리하여 SyntaxError 방지
            # -----------------------------------------------------------------
            format_dict = {
                '현재가': '{:,.0f} 원',
                '등락률': '{:+.2f}%',
            ...
            st.dataframe(
                df_display.style.map(color_signal, subset=['신호']).format(format_dict),
            ...
            with st.expander("📈 차트 보기", expanded=True):
                ...
                for i, (idx, row) in enumerate(df.head(6).iterrows()):
                    with cols[i % 3]:
                        ...
                        fig.add_trace(go.Scatter(x=row['chart_data'].index, y=row['chart_data'], 
                                               mode='lines', line=dict(color=color)))
                        fig.update_layout(title=f"{row['티커']} ({row['등락률']:+.1f}%)", 
                                        height=200, margin=dict(t=30,b=0,l=0,r=0))
                        st.plotly_chart(fig, use_container_width=True)
        
        if errors:
            with st.expander("⚠️ 분석 실패 로그 (Yahoo Finance 차단 등)", expanded=True):
                for e in errors:
                    st.write(e)
            st.warning("일부 데이터 누락됨. (서버 과부하 방지를 위해 천천히 재시도하세요)")
